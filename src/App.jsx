...
export default function App() {
  ...
  async function generateShoppingList() {
    ...
    try {
      ...
      for (const item of items) await sb.upsertShoppingItem(t, { week_start: wk, name: item.name, amount: String(item.amount ?? ""), unit: item.unit || "", category: item.category || "Sonstiges", checked: false, manual: false });
...
function PlanPage({ weekStart, setWeekStart, getMeal, saveMeal, removeMeal, generateShoppingList, loading, pdfLibrary, token, loadPdfLibrary, getValidToken, isMobile }) {
  ...
  const visibleDays = isMobile ? [dayOffset, dayOffset + 1].filter(i => i < 7) : DAYS.map((_, i) => i);
  ...
  return (
    <div>
      ...
      {isMobile && (
        <div style={S.dayNav}>
          <button style={S.dayNavBtn} onClick={() => setDayOffset(o => Math.max(0, o - 2))} disabled={dayOffset === 0}>‹</button>
          <span style={S.dayNavLabel}>
            {fmtFull(addDays(weekStart, dayOffset))} – {fmtFull(addDays(weekStart, Math.min(dayOffset + 1, 6)))}
          </span>
          <button style={S.dayNavBtn} onClick={() => setDayOffset(o => Math.min(5, o + 2))} disabled={dayOffset >= 5}>›</button>
        </div>
      )}

      {/* Days */}
      <div style={{ ...S.daysGrid, gridTemplateColumns: isMobile ? "1fr 1fr" : "repeat(7, 1fr)" }}>
        {visibleDays.map(i => {
...
const globalCss = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  body { background: ${C.bg}; color: ${C.text}; font-family: 'DM Sans', sans-serif; -webkit-tap-highlight-color: transparent; }
...
const S = {
  ...
  dayCol: { background: C.surface, borderRadius: 12, border: `1px solid ${C.border}`, padding: 10, display: "flex", flexDirection: "column", gap: 6 },
  ...
  tile: { background: C.card, borderRadius: 8, border: `1px solid ${C.border}`, padding: 8, minHeight: 58 },
  ...
  slotLabel: { fontSize: 11, color: C.muted, fontWeight: 500 },
  tileActions: { display: "flex", gap: 4 },
  iconBtn: { width: 24, height: 24, borderRadius: 5, border: "none", background: "transparent", color: C.muted, cursor: "pointer", fontSize: 12, display: "flex", alignItems: "center", justifyContent: "center", padding: 0 },
