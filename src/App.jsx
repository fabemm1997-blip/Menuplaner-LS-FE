...
function parseJSON(raw) {
  // Strip markdown code fences and find JSON array
...
// Recipe shape: { id, type: "link"|"pdf", name, link?, pdf_name?, pdf_base64?, recipe_persons }
  ...
  async function generateShoppingList() {
    ...
    try {
      const mealsWithContent = meals.filter(m => {
        try { const r = JSON.parse(m.recipes || "[]"); return r.length > 0; } catch { return false; }
      });
      if (!mealsWithContent.length) { alert("Keine Menüs diese Woche erfasst."); setLoading(false); return; }

      // Build description for each meal+recipe combo
      const lines = [];
      ...
      const prompt = `Du bist ein Schweizer Kochassistent. Erstelle eine vollständige Einkaufsliste für folgende Menüs. Beachte die Personenanzahl: skaliere die Zutaten von "Rezept für X Personen" auf "kochen für Y Personen".

${lines.join("\n")}

Antworte AUSSCHLIESSLICH mit einem JSON-Array, ohne jeglichen Text davor oder danach, ohne Markdown-Formatierung:
[{"name":"Zutat","amount":"200","unit":"g","category":"Gemüse & Früchte"},{"name":"Zutat2","amount":"3","unit":"Stk.","category":"Fleisch & Fisch"}]

Erlaubte Kategorien: Gemüse & Früchte, Fleisch & Fisch, Milchprodukte, Getreide & Backwaren, Hülsenfrüchte, Gewürze & Saucen, Konserven, Tiefkühl, Sonstiges
Erlaubte Einheiten für "unit": g, kg, ml, l, Stk., EL, TL, Bund, Prise, Pkg., Dose
"amount" ist NUR die Zahl als String. Fasse gleiche Zutaten zusammen.`;
      ...
      for (const item of items) {
        await sb.upsertShoppingItem(token, {
          week_start: wk, name: item.name, amount: String(item.amount ?? ""),
          unit: item.unit || "", category: item.category || "Sonstiges", checked: false, manual: false,
        });
      }
      await loadShopping();
      setPage("shopping");
            ...
            <div key={i} style={{ ...styles.dayCol, ...(isToday ? styles.dayColToday : {}) }}>
              ...
              {SLOTS.map(slot => {
                ...
                return <MealTile key={slot} slot={slot} meal={meal} isEdit={isEdit}
                  ...
                  pdfLibrary={pdfLibrary} token={token} loadPdfLibrary={loadPdfLibrary} />;
              })}
            </div>
          );
        })}
...
function MealTile({ slot, meal, isEdit, onEdit, onSave, onRemove, pdfLibrary, token, loadPdfLibrary }) {
  ...
  const [recipes, setRecipes] = useState(() => {
    try { return JSON.parse(meal?.recipes || "[]"); } catch { return []; }
  });
  ...
  async function handlePdf(id, file) {
    ...
    try { await sb.savePdfToLibrary(token, { name: file.name, pdf_base64: base64 }); await loadPdfLibrary(); } catch {}
    try {
      const raw = await callClaude("Extrahiere den Menünamen aus diesem Rezept-PDF. Antworte NUR mit dem Namen des Gerichts, sonst nichts.", base64);
        ...
        <div style={styles.editForm}>
          ...
          {recipes.map((rec, i) => (
            <div key={rec.id} style={styles.recipeBlock}>
              ...
              {rec.type === "link" && (
                <input style={styles.tileInput} placeholder="https://fooby.ch/..." value={rec.link} onChange={e => updateRecipe(rec.id, "link", e.target.value)} />
              )}
              {rec.type === "pdf" && (
              ...
              {rec.type === "library" && (
                <select style={styles.tileSelect} onChange={e => {
                  ...
                  if (item) { updateRecipe(rec.id, "pdf_name", item.name); updateRecipe(rec.id, "pdf_base64", item.pdf_base64); updateRecipe(rec.id, "name", rec.name || item.name.replace(".pdf", "")); }
                }} defaultValue="">
          ...
          <button style={styles.saveBtn} onClick={() => onSave({ persons, also_next_lunch: alsoLunch, recipes: JSON.stringify(recipes) })}>
            Speichern
          </button>
        </div>
      )}
